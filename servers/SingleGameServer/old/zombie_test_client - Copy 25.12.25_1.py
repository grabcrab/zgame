...
class ZombieTestClientApp:
    ...
    def create_ui(self):
        ...
        # Configure row tags for colors - must be done after treeview is created
        # Using more saturated colors for better visibility
        self.device_tree.tag_configure('zombie', background='#ff9999', foreground='#000000')
        self.device_tree.tag_configure('human', background='#99ff99', foreground='#000000')
        self.device_tree.tag_configure('neutral', background='#e0e0e0', foreground='#000000')
        self.device_tree.tag_configure('error', background='#ffcc99', foreground='#000000')
        self.device_tree.tag_configure('low_health', background='#ffb366', foreground='#000000')
    ...
    def stop_all_devices(self):
        ...
        # Signal all simulators to stop (non-blocking)
        for simulator in self.simulators.values():
            simulator.running = False
            
        # Brief wait for threads to notice the stop signal
        time.sleep(0.1)
        
        # Clear simulators without waiting for threads (they're daemon threads)
        self.simulators.clear()
        
        # Reset all devices to neutral/sleep state
        ...
        # Refresh display to show reset states
        self.refresh_device_list()
    ...
    def update_device_row(self, device_id: str):
        ...
        for item in self.device_tree.get_children():
            if self.device_tree.item(item)['values'][0] == device_id:
                ...
                # Prioritize role coloring over error indication
                if device.role == 'zombie':
                    tag = 'zombie'
                elif device.role == 'human':
                    tag = 'human'
                elif device.role == 'neutral':
                    tag = 'neutral'
                else:
                    tag = 'neutral'
                    
                # Override with low_health only if health is critical
                if device.health <= 30 and device.role in ['zombie', 'human']:
                    tag = 'low_health'
