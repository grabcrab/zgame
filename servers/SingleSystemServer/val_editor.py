...
class LEDPatternEditor:
    def __init__(self, root, sync_files_dir=None):
        ...
        # Track selected LEDs: list of (strip_index, led_index) tuples
        self.selected_leds = []
    ...
    def setup_ui(self):
        ...
        # Open/Save buttons at top
        file_btn_frame = ttk.Frame(left_frame)
        file_btn_frame.pack(fill=tk.X, pady=(0, 5))
        
        ttk.Button(file_btn_frame, text="Open", width=8, command=self.open_sync_folder).pack(side=tk.LEFT, padx=2)
        ttk.Button(file_btn_frame, text="Save", width=8, command=self.save_file).pack(side=tk.LEFT, padx=2)
        
        # Pattern listbox with scrollbar
        list_frame = ttk.Frame(left_frame)
        list_frame.pack(fill=tk.BOTH, expand=True)
        
        # BROWSE mode prevents multi-selection, only single selection allowed
        # activestyle='none' removes the underline on the active item
        self.pattern_listbox = tk.Listbox(list_frame, width=20, height=25, 
                                          selectmode=tk.BROWSE, font=('Consolas', 10),
        ...
        self.dup_pattern_btn = ttk.Button(btn_frame, text="Dup", width=4, command=self.duplicate_pattern)
        self.dup_pattern_btn.pack(side=tk.LEFT)
        self.del_pattern_btn = ttk.Button(btn_frame, text="Del", width=4, command=self.delete_pattern)
        self.del_pattern_btn.pack(side=tk.LEFT)
        self.up_pattern_btn = ttk.Button(btn_frame, text="Up", width=3, command=self.move_pattern_up)
        self.up_pattern_btn.pack(side=tk.LEFT)
        self.dn_pattern_btn = ttk.Button(btn_frame, text="Dn", width=3, command=self.move_pattern_down)
        self.dn_pattern_btn.pack(side=tk.LEFT)
        ...
        # Store references to buttons that require selection
        self.delete_frame_btn = ttk.Button(strip_btn_frame, text="Delete Selected", command=self.delete_strip_frame)
        self.delete_frame_btn.pack(side=tk.LEFT, padx=2)
        
        self.duplicate_frame_btn = ttk.Button(strip_btn_frame, text="Duplicate Selected", command=self.duplicate_strip_frame)
        self.duplicate_frame_btn.pack(side=tk.LEFT, padx=2)
        
        self.move_up_btn = ttk.Button(strip_btn_frame, text="Move Up", command=self.move_strip_up)
        self.move_up_btn.pack(side=tk.LEFT, padx=2)
        
        self.move_down_btn = ttk.Button(strip_btn_frame, text="Move Down", command=self.move_strip_down)
        self.move_down_btn.pack(side=tk.LEFT, padx=2)
        
        # Separator
        ttk.Label(strip_btn_frame, text="  |  ").pack(side=tk.LEFT)
        
        # Edit Color button (for selected LEDs)
        self.edit_color_btn = ttk.Button(strip_btn_frame, text="Edit Color", command=self.edit_selected_leds_color)
        self.edit_color_btn.pack(side=tk.LEFT, padx=2)
        
        # Legend / Help text
        legend_frame = ttk.Frame(editor_frame)
        legend_frame.pack(fill=tk.X, pady=(5, 0))
        
        legend_text = "💡 Tip: Click on LED pixels to select them, then press 'Edit Color' to change color for all selected"
        legend_label = ttk.Label(legend_frame, text=legend_text, foreground='#888888', font=('Arial', 9, 'italic'))
        ...
        # Initial button state (disabled until frame selected)
        self.update_frame_buttons_state()
        self.update_pattern_buttons_state()
    ...
    def update_frame_buttons_state(self):
        """Enable/disable frame-related buttons based on selection"""
        has_selection = self.selected_strip_index is not None
        state = 'normal' if has_selection else 'disabled'
        
        self.delete_frame_btn.configure(state=state)
        self.duplicate_frame_btn.configure(state=state)
        self.move_up_btn.configure(state=state)
        self.move_down_btn.configure(state=state)
        
        # Edit Color button - enabled if any LEDs are selected
        has_led_selection = len(self.selected_leds) > 0
        self.edit_color_btn.configure(state='normal' if has_led_selection else 'disabled')
    
    def update_pattern_buttons_state(self):
        """Enable/disable pattern-related buttons based on selection"""
        selection = self.pattern_listbox.curselection()
        has_selection = len(selection) > 0
        state = 'normal' if has_selection else 'disabled'
        
        self.dup_pattern_btn.configure(state=state)
        self.del_pattern_btn.configure(state=state)
        self.up_pattern_btn.configure(state=state)
        self.dn_pattern_btn.configure(state=state)
    ...
    def on_pattern_select(self, event):
        ...
        # Save current pattern before switching (if different)
        if self.current_pattern_index is not None and self.current_pattern_index != new_index and self.data:
            self.save_current_pattern()
            # Re-select the new item after save_current_pattern modified the listbox
            self.pattern_listbox.selection_clear(0, tk.END)
            self.pattern_listbox.selection_set(new_index)
            
        self.current_pattern_index = new_index
    ...
    def save_current_pattern(self):
        ...
        # Update listbox item text (pattern name may have changed)
    ...
    def load_strips(self, strips):
        ...
        # Clear existing widgets and selections
        self.clear_all_led_selections()
        for widget in self.strip_inner_frame.winfo_children():
            widget.destroy()
        self.strip_widgets = []
        self.selected_strip_index = None
        
        # Disable frame buttons until a frame is selected
        self.update_frame_buttons_state()
    ...
    def add_strip_widget(self, index, strip_data=None):
        ...
        widget_data['led_wrappers'] = []  # Store wrapper frames for selection visual
        ...
        for i in range(8):
            ...
            # Wrapper frame for selection highlight (acts as colored border)
            wrapper = tk.Frame(led_frame, bg='#3d3d3d', padx=3, pady=3)
            wrapper.pack(side=tk.LEFT, padx=1)
            widget_data['led_wrappers'].append(wrapper)
            
            btn = tk.Button(wrapper, width=4, height=2, 
                           bg=display_color,
            ...
            # Simple click to toggle LED selection
            btn.bind('<Button-1>', lambda e, idx=len(self.strip_widgets), led=i: self.toggle_led_selection(idx, led))
            wrapper.bind('<Button-1>', lambda e, idx=len(self.strip_widgets), led=i: self.toggle_led_selection(idx, led))
        ...
        # Select button (use padx for better sizing)
        select_btn = tk.Button(frame, text=" Select ", bg='#555555', fg='white',
                              activebackground='#666666', activeforeground='white',
                              padx=10, pady=2,
                              command=lambda idx=len(self.strip_widgets): self.toggle_strip_selection(idx))
        select_btn.pack(side=tk.LEFT, padx=10)
        widget_data['select_btn'] = select_btn
        
        # Click to select/toggle (bind to frame and all child widgets)
        frame.bind('<Button-1>', lambda e, idx=len(self.strip_widgets): self.toggle_strip_selection(idx))
        index_label.bind('<Button-1>', lambda e, idx=len(self.strip_widgets): self.toggle_strip_selection(idx))
        led_frame.bind('<Button-1>', lambda e, idx=len(self.strip_widgets): self.toggle_strip_selection(idx))
        dur_label.bind('<Button-1>', lambda e, idx=len(self.strip_widgets): self.toggle_strip_selection(idx))
        vib_label.bind('<Button-1>', lambda e, idx=len(self.strip_widgets): self.toggle_strip_selection(idx))
        
        self.strip_widgets.append(widget_data)
    
    def toggle_strip_selection(self, index):
        """Toggle strip frame selection - select if not selected, deselect if already selected"""
        if self.selected_strip_index == index:
            # Already selected - deselect it
            self._style_strip_frame(index, selected=False)
            self.selected_strip_index = None
            self.update_frame_buttons_state()
        else:
            # Select this strip
            self.select_strip(index)
    ...
    def _style_strip_frame(self, index, selected=False):
        ...
        # Update LED wrapper backgrounds (only for non-selected LEDs)
        if 'led_wrappers' in widget_data:
            for i, wrapper in enumerate(widget_data['led_wrappers']):
                # Check if this LED is selected
                if (index, i) in self.selected_leds:
                    wrapper.configure(bg='#ff6600')  # Keep orange for selected LEDs
                else:
                    wrapper.configure(bg=bg_color)  # Match strip background
    ...
    def toggle_led_selection(self, strip_index, led_index):
        """Toggle LED selection state - simple click to select/deselect"""
        led_tuple = (strip_index, led_index)
        
        if led_tuple in self.selected_leds:
            # Deselect
            self.selected_leds.remove(led_tuple)
            self.update_led_selection_visual(strip_index, led_index, selected=False)
        else:
            # Select
            self.selected_leds.append(led_tuple)
            self.update_led_selection_visual(strip_index, led_index, selected=True)
        
        # Update status bar with selection info
        count = len(self.selected_leds)
        if count > 0:
            self.status_var.set(f"Selected {count} LED(s). Click more to add, or press 'Edit Color' to change color.")
        else:
            self.status_var.set("Ready")
        
        # Update Edit Color button state
        self.update_frame_buttons_state()
        
        return "break"  # Prevent event propagation
    
    def update_led_selection_visual(self, strip_index, led_index, selected=False):
        """Update the visual appearance of LED selection"""
        widget_data = self.strip_widgets[strip_index]
        wrapper = widget_data['led_wrappers'][led_index]
        
        if selected:
            # Bright orange border for selected LEDs (using wrapper background)
            wrapper.configure(bg='#ff6600')
        else:
            # Normal appearance - match strip frame background
            wrapper.configure(bg='#3d3d3d')
    
    def clear_all_led_selections(self):
        """Clear all LED selections"""
        for strip_index, led_index in self.selected_leds:
            if strip_index < len(self.strip_widgets):
                self.update_led_selection_visual(strip_index, led_index, selected=False)
        self.selected_leds.clear()
        self.update_frame_buttons_state()
    
    def edit_selected_leds_color(self):
        """Open color picker and apply to all selected LEDs"""
        if not self.selected_leds:
            return
        
        # Use the first selected LED's color as initial
        first_strip, first_led = self.selected_leds[0]
        current = self.strip_widgets[first_strip]['colors'][first_led].get()
        initial = self.hex_to_rgb(current)
        
        count = len(self.selected_leds)
        color = colorchooser.askcolor(color=initial, 
                                      title=f"Pick color for {count} selected LED(s)")
        ...
        if color[0]:
            ...
            # Update preview if any affected strip is selected
            if self.selected_strip_index is not None:
                for strip_index, _ in self.selected_leds:
                    if strip_index == self.selected_strip_index:
                        self.select_strip(self.selected_strip_index)
                        break
            
            self.status_var.set(f"Applied color to {count} LED(s)")
    ...
    def draw_leds(self, strips, frame_index=0):
        ...
        for i in range(8):
            if i < len(strip):
                raw_color = strip[i]
                # Use display color (brightened for visibility)
                color = self.get_display_color(raw_color)
            else:
                color = "#000000"
