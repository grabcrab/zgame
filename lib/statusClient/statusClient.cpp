...
// ============== Internal State ==============
static char _deviceName[STATUS_DEVICE_NAME_MAX_LEN + 1] = {0};
...
// Preferences namespace for storing device name
static const char* PREFS_NAMESPACE = "statusClient";
static const char* PREFS_KEY_NAME = "deviceName";
...
static bool checkForNewName(const String& response, char* newNameOut, size_t maxLen);
...
static void generateDefaultName(char* buffer, size_t bufferSize);
static bool loadNameFromPreferences(void);
static bool saveNameToPreferences(const char* name);

// ============== Public Functions ==============

bool statusClientInit(const char* serverIP)
{
    if (serverIP == NULL)
    {
        Serial.println("!!! statusClientInit: NULL serverIP parameter");
        return false;
    }
    ...
    // Load device name from Preferences, or generate default if not saved
    if (!loadNameFromPreferences())
    {
        // Generate default name: BAZA_XXXXXXX where XXXXXXX is zero-padded MAC in hex
        generateDefaultName(_deviceName, sizeof(_deviceName));
        Serial.printf(">>> statusClientInit: Generated default name: <%s>\n", _deviceName);
        
        // Save the generated name
        saveNameToPreferences(_deviceName);
    }
    else
    {
        Serial.printf(">>> statusClientInit: Loaded name from Preferences: <%s>\n", _deviceName);
    }
...
const char* statusClientGetName(void)
{
    static bool loaded = false;
    if (!loaded)
    {
        loaded = loadNameFromPreferences();
    }
    if (loaded)
    {
        return _deviceName;
...
static void generateDefaultName(char* buffer, size_t bufferSize)
{
    ...
    // Generate name: BAZA_ + last 3 bytes of MAC in hex (zero-padded, 7 chars total)
    // Format: BAZA_XXXXXX where XXXXXX is the last 3 MAC bytes
    snprintf(buffer, bufferSize, "BAZA_%02X%02X%02X", mac[3], mac[4], mac[5]);
}
...
static bool loadNameFromPreferences(void)
{
    ...
    {
        Serial.println("!!! statusClient: Failed to open Preferences for reading");
        return false;
    }
    
    String storedName = prefs.getString(PREFS_KEY_NAME, "");
    prefs.end();
    
    if (storedName.length() == 0 || storedName.length() > STATUS_DEVICE_NAME_MAX_LEN)
    {
        return false;  // No valid name stored
    }
    
    strncpy(_deviceName, storedName.c_str(), STATUS_DEVICE_NAME_MAX_LEN);
    _deviceName[STATUS_DEVICE_NAME_MAX_LEN] = '\0';
...
static bool saveNameToPreferences(const char* name)
{
    ...
    {
        Serial.println("!!! statusClient: Failed to open Preferences for writing");
        return false;
    }
    
    size_t written = prefs.putString(PREFS_KEY_NAME, name);
    prefs.end();
    
    if (written == 0)
    {
        Serial.println("!!! statusClient: Failed to save name to Preferences");
        return false;
    }
    
    Serial.printf(">>> statusClient: Saved name to Preferences: <%s>\n", name);
    return true;
}
...
static bool sendStatusUpdate(void)
{
    ...
    if (httpCode > 0)
    {
        if (httpCode == HTTP_CODE_OK)
        {
            ...
            // Check for new name from server
            char newName[STATUS_DEVICE_NAME_MAX_LEN + 1];
            if (checkForNewName(response, newName, sizeof(newName)))
            {
                // Only update if the name is different
                if (strcmp(newName, _deviceName) != 0)
                {
                    Serial.printf(">>> StatusClient: Received new name from server: <%s>\n", newName);
                    
                    // Update the device name in memory
                    strncpy(_deviceName, newName, STATUS_DEVICE_NAME_MAX_LEN);
                    _deviceName[STATUS_DEVICE_NAME_MAX_LEN] = '\0';
                    
                    // Save to Preferences
                    saveNameToPreferences(newName);
                    
                    // Trigger an immediate update to confirm the new name to server
                    doUpdate = true;
                }
            }
...
static bool checkForNewName(const String& response, char* newNameOut, size_t maxLen)
{
    ...
    const char* newName = doc["new_name"];
    if (newName == NULL || strlen(newName) == 0)
    {
        return false;
    }
    
    strncpy(newNameOut, newName, maxLen - 1);
    newNameOut[maxLen - 1] = '\0';
