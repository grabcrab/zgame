...
void tftProcessMainScreen(tTftMainScreenRecord *dRec)
{
    ...
    spr.drawString(vccStr, WIDTH, 0, VCC_FONT);

    spr.setTextSize(0);
    spr.setTextDatum(BR_DATUM);
    spr.drawString(idStr, WIDTH, HEIGHT, SELF_ID_FONT);

    spr.setTextDatum(TC_DATUM);
    spr.setTextSize(2);
    spr.drawString(rssiStr, WIDTH/2 - 25, 10, RSSI_FONT);
    spr.setTextSize(1);
    spr.drawString(dNumStr, WIDTH/2, 115, REMOTE_ID_FONT);
...
void tftBootScreen(void)
{
    ...
    spr.setTextSize(2);
    spr.drawString("HOLD", WIDTH/2, HEIGHT/2, BOOT_FONT);
    lcd_PushColors(0, 0, WIDTH, HEIGHT, (uint16_t *)spr.getPointer());
}
...
void tftSleepScreen(void)
{
    ...
    spr.setTextDatum(MC_DATUM);
    spr.setTextSize(2);
    spr.drawString("SLEEP", WIDTH/2, HEIGHT/2, BOOT_FONT);
    lcd_PushColors(0, 0, WIDTH, HEIGHT, (uint16_t *)spr.getPointer());
}
